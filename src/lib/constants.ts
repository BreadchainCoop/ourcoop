...
/**
 * The one canonical CrowdStakeDeployer — one-tx full-instance deployer
 * (admin/democratic registry + instance metadata). Uses a *static* process.env
 * reference (not the dynamic env() helper) so a NEXT_PUBLIC_DEPLOYER_ADDRESS
 * override is actually inlined into the client bundle.
 */
